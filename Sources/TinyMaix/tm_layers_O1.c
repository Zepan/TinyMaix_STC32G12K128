...
#define BATCH_SIZE 1 // batch sum size
/*************************** TML_CONV2D **********************************/
...
// 1x1 pw conv
TM_INLINE tm_err_t l_tml_pwconv2d(tm_mat_t *in, tm_mat_t *out, wtype_t *w,
                                  btype_t *b, int kw, int kh, int sx, int sy,
                                  int dx, int dy, int act, int pad_top,
                                  int pad_bottom, int pad_left, int pad_right,
                                  int dmul, sctype_t *ws, sctype_t in_s,
                                  zptype_t in_zp, sctype_t out_s,
                                  zptype_t out_zp) {
  int pad_flag =
  ...
  for (int y = 0; y < out->h; y++) {
    for (int x = 0; x < out->w; x++) {
      ...
      for (; c < out->c - BATCH_SIZE + 1;) {
        ...
        l_postprocess_sum(sums[0], b[c], act, outp, SUMSCALE, outscale, out_zp);
        c++;
        outp++;
        l_postprocess_sum(sums[1], b[c], act, outp, SUMSCALE, outscale, out_zp);
        c++;
        outp++;
...
// normal conv
TM_INLINE tm_err_t l_tml_conv2d(tm_mat_t *in, tm_mat_t *out, wtype_t *w,
                                btype_t *b, int kw, int kh, int sx, int sy,
                                int dx, int dy, int act, int pad_top,
                                int pad_bottom, int pad_left, int pad_right,
                                int dmul, sctype_t *ws, sctype_t in_s,
...
// dwconv
TM_INLINE tm_err_t l_tml_dwconv2d(tm_mat_t *in, tm_mat_t *out, wtype_t *w,
                                  btype_t *b, int kw, int kh, int sx, int sy,
                                  int dx, int dy, int act, int pad_top,
                                  int pad_bottom, int pad_left, int pad_right,
                                  int dmul, sctype_t *ws, sctype_t in_s,
      ...
      if (!slow_flag) { // valid or same valid part
        ...
        if (maxk == 9) {
          ...
          for (int c = 0; c < out->c; c++) {
            ...
            // sum = sptr[0]*kptr[0] + sptr[1]*kptr[1] + sptr[2]*kptr[2] +
...
// dwconv
TM_INLINE tm_err_t l_tml_dwconv2d_3x3_part(
    ...
    sctype_t out_s, zptype_t out_zp, int x0, int x1, int y0, int y1) {
  int pad_flag =
      (pad_top != 0 || pad_bottom != 0 || pad_left != 0 || pad_right != 0);
  int maxk = kw * kh;
  int chi = in->c;
...
// 40ms->27ms
TM_INLINE tm_err_t l_tml_dwconv2d_3x3_nostride(
    tm_mat_t *in, tm_mat_t *out, wtype_t *w, btype_t *b, int kw, int kh, int sx,
    int sy, int dx, int dy, int act, int pad_top, int pad_bottom, int pad_left,
    int pad_right, int dmul, sctype_t *ws, sctype_t in_s, zptype_t in_zp,
  ...
  for (int y = 0; y + CONV_BLK_STEPY <= out->h; y += CONV_BLK_STEPY) {
    ...
    for (int x = 0; x + CONV_BLK_STEPX <= out->w; x += CONV_BLK_STEPX) {
      int src_x0 = sx * x - pad_left;
      sumtype_t sum;
      slow_flag = ((src_y0 < 0) + (src_x0 < 0) + (src_y0 + CONV_BLK_H > in->h) +
      ...
      if (!slow_flag) { // valid or same valid part
        ...
        for (int c = 0; c < cho; c++) {
          wtype_t *kptr = (wtype_t *)w + c * maxk;
          sum0 = sptr[dw_koft[0]] * kptr[0] + sptr[dw_koft[1]] * kptr[1] +
                 sptr[dw_koft[2]] * kptr[2] + sptr[dw_koft[4]] * kptr[3] +
                 sptr[dw_koft[5]] * kptr[4] + sptr[dw_koft[6]] * kptr[5] +
                 sptr[dw_koft[8]] * kptr[6] + sptr[dw_koft[9]] * kptr[7] +
                 sptr[dw_koft[10]] * kptr[8];
          sum1 = sptr[dw_koft[1]] * kptr[0] + sptr[dw_koft[2]] * kptr[1] +
                 sptr[dw_koft[3]] * kptr[2] + sptr[dw_koft[5]] * kptr[3] +
                 sptr[dw_koft[6]] * kptr[4] + sptr[dw_koft[7]] * kptr[5] +
                 sptr[dw_koft[9]] * kptr[6] + sptr[dw_koft[10]] * kptr[7] +
                 sptr[dw_koft[11]] * kptr[8];
          sum2 = sptr[dw_koft[4]] * kptr[0] + sptr[dw_koft[5]] * kptr[1] +
                 sptr[dw_koft[6]] * kptr[2] + sptr[dw_koft[8]] * kptr[3] +
                 sptr[dw_koft[9]] * kptr[4] + sptr[dw_koft[10]] * kptr[5] +
                 sptr[dw_koft[12]] * kptr[6] + sptr[dw_koft[13]] * kptr[7] +
                 sptr[dw_koft[14]] * kptr[8];
          sum3 = sptr[dw_koft[5]] * kptr[0] + sptr[dw_koft[6]] * kptr[1] +
                 sptr[dw_koft[7]] * kptr[2] + sptr[dw_koft[9]] * kptr[3] +
                 sptr[dw_koft[10]] * kptr[4] + sptr[dw_koft[11]] * kptr[5] +
                 sptr[dw_koft[13]] * kptr[6] + sptr[dw_koft[14]] * kptr[7] +
                 sptr[dw_koft[15]] * kptr[8];
          l_postprocess_sum(sum0, b[c], act, outp + 0 * cho, SUMSCALE, outscale,
                            out_zp);
          l_postprocess_sum(sum1, b[c], act, outp + 1 * cho, SUMSCALE, outscale,
                            out_zp);
          l_postprocess_sum(sum2, b[c], act, outp + (out->w + 0) * cho,
                            SUMSCALE, outscale, out_zp);
          l_postprocess_sum(sum3, b[c], act, outp + (out->w + 1) * cho,
                            SUMSCALE, outscale, out_zp);
          outp++;
          sptr++;
        }
...
#error "unsupport mdl type"
        ...
        for (int c = 0; c < out->c; c++) {
          ...
          // l_postprocess_sum(sum, b[c], act, outp+0*cho, SUMSCALE, outscale,
          // out_zp);
  ...
  if (out->w % CONV_BLK_W != 0 || out->h % CONV_BLK_H != 0) { // cal rest part
    ...
    l_tml_dwconv2d_3x3_part(in, out, w, b, kw, kh, sx, sy, dx, dy, act, pad_top,
                            pad_bottom, pad_left, pad_right, dmul, ws, in_s,
                            in_zp, out_s, out_zp, out->w - out->w % CONV_BLK_W,
                            out->w, 0, out->h / CONV_BLK_H * CONV_BLK_H);
    l_tml_dwconv2d_3x3_part(
        in, out, w, b, kw, kh, sx, sy, dx, dy, act, pad_top, pad_bottom,
        pad_left, pad_right, dmul, ws, in_s, in_zp, out_s, out_zp, 0, out->w,
        out->h - out->h % CONV_BLK_H, out->h / CONV_BLK_H * CONV_BLK_H);
  }
...
TM_INLINE tm_err_t l_tml_mdwconv2d(tm_mat_t *in, tm_mat_t *out, wtype_t *w,
                                   btype_t *b, int kw, int kh, int sx, int sy,
                                   int dx, int dy, int act, int pad_top,
                                   int pad_bottom, int pad_left, int pad_right,
                                   int dmul, sctype_t *ws, sctype_t in_s,
...
tm_err_t
tml_conv2d_dwconv2d(tm_mat_t *in, tm_mat_t *out, wtype_t *w, btype_t *b, int kw,
                    int kh, int sx, int sy, int dx, int dy, int act,
                    int pad_top, int pad_bottom, int pad_left, int pad_right,
                    int dmul, sctype_t *ws, sctype_t in_s, zptype_t in_zp,
                    sctype_t out_s, zptype_t out_zp) // kernel: (cho, chi, h, w)
{
  ...
  if (maxk == 1) {
    ...
    l_tml_conv2d(in, out, w, b, kw, kh, sx, sy, dx, dy, act, pad_top,
                 pad_bottom, pad_left, pad_right, dmul, ws, in_s, in_zp, out_s,
                 out_zp);
    TM_PERF_ADD(t_conv);
  } else if (dmul == 1) {
...
/*************************** TML_SOFTMAX **********************************/
tm_err_t TM_WEAK tml_softmax(tm_mat_t *in, tm_mat_t *out, sctype_t in_s,
                             zptype_t in_zp, sctype_t out_s, zptype_t out_zp) {
