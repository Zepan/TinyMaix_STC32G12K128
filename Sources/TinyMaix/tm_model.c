...
// layer callback; in: return input mat, include buf addr; //you can ignore it if
// use static buf

tm_err_t TM_WEAK tm_load(tm_mdl_t *mdl, const uint8_t *bin, uint8_t *buf,
                         tm_cb_t cb, tm_mat_t *in) {
	char* tmp;
  tm_mdlbin_t *mdl_bin = (tm_mdlbin_t *)bin;

  tmp = (char*)mdl_bin;
  // if(mdl_bin->magic != TM_MDL_MAGIC)   return TM_ERR_MAGIC;   //FIXME:
  // big-endian not compatible
  ...
  in->dat = (mtype_t *)mdl->buf; // input at 0 oft
  return TM_OK;
}
...
// preprocess dat input
tm_err_t TM_WEAK tm_preprocess(tm_mdl_t *mdl, tm_pp_t pp_type, tm_mat_t *in,
                               tm_mat_t *out) {
  int i;uint8_t tmp;
  ...
  case TMPP_FP2INT:
    for (i = 0; i < in_size; i++)
      out->dat[i] = (mtype_t)(((float *)in->dat)[i] / in_s + in_zp);
    break;
  case TMPP_UINT2INT:
    for (i = 0; i < in_size; i++){
			out->dat[i] = ((mtype_t)((((uint8_t *)(in->dat))[i]) - 128))
                    << UINT2INT_SHIFT;
		}
    break;
#else
...
tm_err_t TM_WEAK tm_run(tm_mdl_t *mdl, tm_mat_t *in, tm_mat_t *out) {
  ...
  for (mdl->layer_i = 0; mdl->layer_i < mdl->b->layer_cnt; mdl->layer_i++) {
    ...
    if (mdl->layer_i > 0) {
      _in.dat = (mtype_t *)(mdl->buf + h->in_oft);
      memcpy((void *)&_in, (void *)(h->in_dims), sizeof(uint16_t) * 4);
    }
    _out.dat = (mtype_t *)(mdl->buf + h->out_oft);
    ...
    case TML_DWCONV2D: {
      ...
      res = tml_conv2d_dwconv2d(
          ...
          (btype_t *)(mdl->layer_body + l->b_oft), l->kernel_w, l->kernel_h,
          l->stride_w, l->stride_h, l->dilation_w, l->dilation_h, l->act,
          l->pad[0], l->pad[1], l->pad[2], l->pad[3], (int)l->depth_mul,
    ...
    case TML_FC: {
      ...
      res = tml_fc(&_in, &_out, (wtype_t *)(mdl->layer_body + l->w_oft),
